...
def is_valid_solution(G, c, k):
    ...
    for road in k:
        assert H.has_edge(
            road[0], road[1]), "Invalid Solution: {} is not a valid edge in graph G".format(road)
    H.remove_edges_from(k)

    for city in c:
        assert H.has_node(
            city), "Invalid Solution: {} is not a valid node in graph G".format(city)
...
def move_better_outputs(set_size, new_output_path, old_output_path):
    '''
    Compare results of old and new outputs generated from the same input.
    Then switch old outputs to new outputs only if new outputs have better results.
    Args:
        set_size: size of outputs to compare i.e. small or medium or large
        new_output_path: directory name of which new outputs are stored
        old_output_path: directory name of which old outputs are stored
